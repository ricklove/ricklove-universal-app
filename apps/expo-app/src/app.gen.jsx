...
var recordRun = (dataset) => {
  dataset.allNodeInstances.forEach((node) => {
    ...
    runValue.key = `${[
      ...runValue.inputs.map((x) => `${x.value}`.substring(0, 256)),
      ...runValue.inner.map((x) => `${x.value}`.substring(0, 256)),
      ...runValue.outputs.map((x) => `${x.value}`.substring(0, 256))
    ].join(`,`)}`;
    (node.runs = node.runs ?? []).push(runValue);
  });
};
