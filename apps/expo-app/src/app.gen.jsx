// ../../packages/pipescript/src/tests/code/0002-assignments/01.ts.workflow.json
...
var NodeConnection = ({
  ...
  return <View5 className="flex-row justify-start items-center">
    <View5 className="flex-column">{connectionsIn.map((x) => <React4.Fragment key={x.key}>
    ...
    <View5 className="flex-column">{connectionsOut.map((x) => <React4.Fragment key={x.key}>
      ...
      {x.outflowPipes.map((outflowPipe) => outflowPipe && <React4.Fragment key={outflowPipe.key}><PipeValueView pipeValue={outflowPipe} /></React4.Fragment>)}
    </React4.Fragment>)}</View5>
  </View5>;
};
...
var loadNodeConnections_inflows = (nodeInstance) => {
  nodeInstance.inputs.forEach((input) => {
    ...
    const getInflowSource = () => {
      ...
      if (pipeSource.kind === `node`) {
        const peerNodeOutput = nodeInstance.children.find((n2) => n2.node.nodeId === pipeSource.sourceNodeId)?.outputs.find((x) => x.name === pipeSource.name);
        if (!peerNodeOutput) {
          console.warn(
  ...
  nodeInstance.outputs.forEach((output) => {
    ...
    const getInflowSource = () => {
      ...
      if (pipeSource.kind === `node`) {
        const peerNodeOutput = nodeInstance.children.find((n2) => n2.node.nodeId === pipeSource.sourceNodeId)?.outputs.find((x) => x.name === pipeSourceName);
        if (!peerNodeOutput) {
          console.warn(
