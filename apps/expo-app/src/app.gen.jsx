...
import React3, { useLayoutEffect as useLayoutEffect2, useRef as useRef4, useState as useState3 } from "react";
import { View as View3, Text as Text3, Pressable as Pressable3 } from "react-native";
...
var WorkCanvasView = ({ workflow }) => {
  ...
  const [tab, setTab] = useState3(`work-flow`);
  return <View3 className="bg-slate-900 w-full h-full overflow-hidden"><MoveableView
    ...
    HeaderComponent={useStableCallback(() => <View3 className="flex-row">{tabs.map((x) => <React3.Fragment key={x}><Pressable3 onPress={() => setTab(x)}><View3 className={`p-1 m-1 border-solid border-[1px] border-gray-800 ${tab === x ? `` : `opacity-50`}`}><Text3
      className={`text-gray-800 ${tab === x ? `` : `opacity-50`}`}
    >{x}</Text3></View3></Pressable3></React3.Fragment>)}</View3>)}
  ><View3 ref={viewRef} className="w-full h-full justify-center items-center"><PipeEndpointsRegistry.Provider value={context2.current}>{tab === `work-flow` && <WorkFlowView workflow={workflow} full />}</PipeEndpointsRegistry.Provider></View3></MoveableView></View3>;
};
