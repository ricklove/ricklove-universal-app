...
var convertWorkflowToFunctionDeclaration = (workflow, dataset, builder = {
  ...
  const statements = workflow.body.nodes.map((node) => {
    ...
    const outputsExpression = outputsItems.length === 1 ? `${outputsItems[0]}` : outputsItems.length ? `{ ${outputsItems.join(`, `)} }` : `/* missing output item */ _unknown`;
...
var Footer = ({ nodeInstance }) => {
  const tabs = [`sheet`, `code`, `both`];
  const [tab, setTab] = useState4(`sheet`);
  return <>
